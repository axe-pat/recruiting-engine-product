...
`GET /api/v1/operator/overview` is the paired cockpit read model. It combines capability state, sanitized installed-engine assets, and the ten most recent audited operator jobs. The more focused capabilities, assets, and job endpoints return the same canonical objects without requiring clients to fetch raw companion resources.
...
- `nightly.run`, `outreach.send`, and `application.submit` are explicitly forbidden.

Every executable action requires its exact capability-specific confirmation phrase, re-checks availability and lock state, rejects symlinks and out-of-root targets, and uses an argument vector with `shell=False`. Local-write/model actions create an audited job immediately and run in a daemon worker. The worker holds the shared runtime `operator_mutation.lock` for the entire subprocess, requires every upstream lock to be free at start, and enforces a command-specific timeout. The production pipeline acquires the same lock after its pipeline lock, so a pipeline that starts after the cockpit probe waits instead of racing the local write. The persistent audit row records only validated parameters, command/status/scope/timestamps, the argv hash, lock states, return code, and hashes/line counts for output. It never stores or returns stdout/stderr. The API accepts no caller path, flag, environment override, shell text, recipient, model, limit, or content.
