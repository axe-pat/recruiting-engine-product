...
export function AppFrame({ view }: { view: AppView }) {
  ...
  const loadDashboard = useCallback(async (nextConfig: CompanionConfig) => {
    try {
      ...
      const [payload, preferences] = await Promise.all([
        apiRequest<unknown>(nextConfig, "/api/v1/dashboard"),
        apiRequest<{ preferences?: Record<string, unknown> }>(nextConfig, "/api/v1/preferences"),
      ]);
      const nextSnapshot = safeSnapshot(payload);
      if (!nextSnapshot) throw new Error("The companion returned an incompatible dashboard payload.");
      ...
      if (nextMode === "existing") {
        [existing, existingEvidence] = await Promise.all([
  ...
  const runEngine = async () => {
    ...
    if (workspaceMode === "existing") {
      setRunning(true);
      setNotice("Refreshing exact existing-engine evidence. The signed scheduler remains the only live-run owner.");
  ...
  const changeWorkspaceMode = async (nextMode: "portable" | "existing") => {
    ...
    try {
      ...
      setNotice(nextMode === "existing" ? "Existing-engine evidence mode enabled. Live execution remains scheduler-owned." : "Portable local workspace enabled.");
      await loadDashboard(config);
    } catch (error) {
      ...
      <section className="app-main">
        <header className="app-topbar">
          ...
          <div className="topbar-actions">
            ...
            <button className="run-button" type="button" onClick={runEngine} disabled={running} aria-label={workspaceMode === "existing" ? "Refresh existing-engine evidence" : "Run the portable engine"}>
              <span>{running ? (workspaceMode === "existing" ? "Refreshing" : "Running") : (workspaceMode === "existing" ? "Refresh evidence" : "Run engine")}</span>
        ...
        <div className="app-content">
          {view === "dashboard" ? <Dashboard snapshot={snapshot} mode={mode} workspaceMode={workspaceMode} existingEngine={existingEngine} existingSnapshot={existingSnapshot} /> : null}
          {view === "sources" ? <SourcesView snapshot={snapshot} onImport={importJobs} /> : null}
          {view === "queue" ? (workspaceMode === "existing" ? <ExistingModePage view={view} evidence={existingSnapshot} /> : <QueueView snapshot={snapshot} />) : null}
          {view === "runs" ? (workspaceMode === "existing" ? <ExistingModePage view={view} evidence={existingSnapshot} /> : <RunsView snapshot={snapshot} />) : null}
          {view === "applications" ? (workspaceMode === "existing" ? <ExistingModePage view={view} evidence={existingSnapshot} /> : <ApplicationsView snapshot={snapshot} />) : null}
          {view === "outreach" ? (
            workspaceMode === "existing" ? <ExistingModePage view={view} evidence={existingSnapshot} /> : <OutreachView snapshot={snapshot} onApprove={approveOutreach} />
          ) : null}
          {view === "reports" ? (workspaceMode === "existing" ? <ExistingModePage view={view} evidence={existingSnapshot} /> : <ReportsView snapshot={snapshot} />) : null}
          {view === "settings" ? (
            <SettingsView
...
function ExistingModePage({ view, evidence }: { view: AppView; evidence: ExistingEngineSnapshot | null }) {
  const labels: Partial<Record<AppView, [string, string]>> = {
    queue: ["Verified queue evidence", "Exact queue aggregates without borrowing current workspace rows."],
    runs: ["Production run evidence", "One terminal summary, its exact manifest, and its run-scoped report."],
    applications: ["Current application state", "Mutable application counts remain separately labeled and lock-protected."],
    outreach: ["Current relationship state", "The public-origin UI receives aggregate counts—not contacts, drafts, or messages."],
    reports: ["Trustworthy reports", "The adapter surfaces only a fully verified run-scoped report."],
  };
  const [title, body] = labels[view] ?? ["Existing-engine evidence", "Read-only production evidence."];
  return (
    <section className="app-panel full-panel existing-mode-page">
      <PageLead eyebrow="Read-only existing adapter" title={title} body={body} />
      <ExistingEvidencePanel evidence={evidence} />
    </section>
  );
}
    ...
    <div className="settings-layout">
      ...
      <section className="app-panel settings-card">
        ...
        <div className="mode-cards">
          ...
          <button className={workspaceMode === "existing" ? "active" : ""} type="button" aria-pressed={workspaceMode === "existing"} onClick={() => onModeChange("existing")}><span>02</span><strong>Existing engine</strong><p>Exact nightly pipeline, manifests, queues, and reports surfaced through a read-only local adapter.</p></button>
        </div>
        ...
        <div className="binding-status-card">
          ...
          <div><strong>Existing evidence · {existingReady ? "verified" : "not ready"}</strong><small>{existingEngine?.verified_run_count ?? 0} exact runs verified · guard {existingEngine?.production_guard ?? "unavailable"} · UI mode {workspaceMode}</small></div>
        </div>
